...
- 금지: 성공 기준 없는 모호한 작업 금지, 재현 테스트 없는 즉시 수정 금지

### Response Guidelines

- 버전 태깅: 모든 기술 응답은 `[YY-MM-DD hh:mm:ss] vX`로 시작하여 시간 컨텍스트 정렬
...
0. 가정 명시: 구현 전 가정을 나열. 불확실하면 사용자에게 확인
    ...
    - 더 간단한 방법이 있으면 먼저 제안
    - 이해 안 되면 멈추고 질문
    - 범위/포맷/볼륨을 임의로 단정하거나 latency/throughput 등 목적을 조용히 선택하지 않음
